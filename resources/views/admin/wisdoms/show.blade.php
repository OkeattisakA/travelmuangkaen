        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <strong>{{ $wisdom->wisdom_name_th }} | {{ $wisdom->wisdom_name_eng }}
                        ...
                        @if($wisdom->wisdom_switch === 1)
                            <span class="badge badge-success">เปิดการแสดงผล</span>
                        @else
                            <span class="badge badge-warning">ปิดการแสดงผล</span>
                        @endif

                    </strong>
                    <div class="pull-right"><a href="{{ url("admin/wisdoms/{$wisdom->wisdom_id}/edit") }}" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="แก้ไข"></i></a></div>

                    @if($wisdom->wisdom_switch === 1)
                        <div class="pull-right"><a href="{{ url("admin/wisdoms/switch/0/{$wisdom->wisdom_id}") }}" class="btn btn-warning"><i class="fa fa-eye-slash" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="ปิดการแสดงผล"></i></a></div>
                    @else
                        <div class="pull-right"><a href="{{ url("admin/wisdoms/switch/1/{$wisdom->wisdom_id}") }}" class="btn btn-success"><i class="fa fa-eye" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="เปิดการแสดงผล"></i></a></div>
                    @endif

                </div>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active">เพิ่มโดย : {{ $wisdom->user->name }} | เมื่อวันที่ {{ $wisdom->created_at }}</li>
                </ol>
                    ...
                    <div class="card-body">
                        ...
                        <table id="reviewtable" class="table table-bordered table-striped">
                            ...
                            @foreach ($wisdom->wisdomreviews as $wisdomreview)
                                <tr>
                                    <td>
                                        <a href="https://www.facebook.com/{{ $wisdomreview->user->provider_id }}"><img src="http://graph.facebook.com/{{ $wisdomreview->user->provider_id }}/picture" class="img-avatar"></a>
                                    ...
                                    <td>{{ $wisdomreview->wisdomreview_detail }}</td>
                                    <td align="center">
                                        <a href="{{ asset("uploads/reviews/$wisdomreview->wisdomreview_photo") }}" data-lightbox="{{ $wisdomreview->wisdomreview_id }}">
                                            <img src="{{ asset("uploads/reviews/$wisdomreview->wisdomreview_photo") }}" alt="" style="width: 50px; height: 50px;" class="img-fluid img-thumbnail">
                                        </a>
                                    </td>
                                    <td>{{ $wisdomreview->created_at }}</td>
                                    @if($wisdomreview->wisdomreview_status ==1)
                                        <td>
                                            <span class="badge badge-success">ตรวจสอบแล้ว</span>
                                        </td>
                                        <td>
                                            {!! Form::open(['method' => 'DELETE','style'=>'display:inline-block','action' => ['Admin\WisdomreviewController@destroy',$wisdomreview->wisdomreview_id]]) !!}

                                            <button type="submit" onclick="return confirm('Are you sure?')" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="ลบรีวิว"><i class="fa fa-trash-o"></i></button>
                                    ...
                                    @else
                                        ...
                                        <td>
                                            <a class="btn btn-success" href="{{ url("admin/wisdomreviews/approve/{$wisdom->wisdom_id}/{$wisdomreview->wisdomreview_id}") }}" role="button" data-toggle="tooltip" data-placement="bottom" title="อนุมัติ"><i class="fa fa-check"></i></a>
                                        </td>
                                    @endif
                ...
                @if($wisdom->wisdom_status === 0)
                    <div class="text-center">
                        <a href="{{ url("admin/wisdoms/approve/{$wisdom->wisdom_id}") }}" class="btn btn-success">อนุมัติ</a>
                    </div>
                @else
